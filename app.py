...
try:
    ...
    st.error("Error: 'scaler.pkl' or 'elastic_net_model.pkl' not found.")
    st.stop()
except pickle.PickleError:
    st.error("Error: Failed to load 'scaler.pkl' or 'elastic_net_model.pkl'. The files might be corrupted.")
    st.stop()
...
def run_ml_app():
    ...
    if button:
        ...
        if data.shape[1] == 26:
            ...
            # Making prediction
            prediction = elastic_net_model.predict(data_scaled)
